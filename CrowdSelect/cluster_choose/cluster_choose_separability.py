...
separability_top_1k_cluster_path = os.path.join(base_path,"data/separability_1k_top_30_clusters.json")
separability_bottom_1k_cluster_path = os.path.join(base_path,"data/separability_1k_bottom_30_clusters.json")
=======
...
separability_top_1k_cluster_path = r"..\..\data\cluster_choose\30\separability_1k_top_30_clusters.json"
separability_bottom_1k_cluster_path = r"..\..\data\cluster_choose\30\separability_1k_bottom_30_clusters.json"
...
# Load separability scores
with open(separability_score_path, 'r') as f:
    separability_scores = json.load(f)
...
uuid_to_score = {item['uuid']: item['separability_score'] for item in separability_scores}
...
for cluster in clusters:
    # Sort the cluster by separability_score in descending order and select the top 34
    sorted_cluster = sorted(cluster, key=lambda uuid: uuid_to_score[uuid], reverse=True)
    selected_top_uuids.extend(sorted_cluster[:34])

    # Sort the cluster by separability_score in ascending order and select the bottom 34
    sorted_cluster_low = sorted(cluster, key=lambda uuid: uuid_to_score[uuid])
    selected_bottom_uuids.extend(sorted_cluster_low[:34])

# Sort the selected top UUIDs by separability_score in descending order and take the top 1000
...
# Sort the selected bottom UUIDs by separability_score in ascending order and take the bottom 1000
...
# Print the separability scores of the first UUID in the top and bottom 1k
print(uuid_to_score[top_1000_uuids])
print(uuid_to_score[bottom_1000_uuids])

# Save the results to JSON files
with open(separability_top_1k_cluster_path, 'w') as f:
    json.dump(cluster_top_1k, f)

with open(separability_bottom_1k_cluster_path, 'w') as f:
