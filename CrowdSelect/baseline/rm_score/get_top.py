...
output_path=os.path.join(base_path,"data/rm_top.json")
...
top_scores = []
for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Finding top scores"):
...
for _, row in tqdm(top_1k_df.iterrows(), total=top_1k_df.shape[0], desc="Extracting responses"):
    uuid = row['uuid']
    instruction = row['instruction']
