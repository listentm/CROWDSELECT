...
output_file = os.path.join(base_path,"data/length_1k_top.json")

# Load the dataset
ds = load_dataset("Magpie-Align/Magpie-100K-Generator-Zoo", cache_dir=magpie_path)
...
# Sort by token length and take the top 1000 longest instructions
instructions = sorted(instructions, key=lambda x: x["token_length"], reverse=True)
top_1000_instructions = instructions[:1000]

# Find the longest response for each instruction
result = []
model_keys = [
...
for item in tqdm(top_1000_instructions, desc="Processing top 1000 instructions"):
    idx = item["index"]
    instruction = item["instruction"]
    ...
    for model_key in model_keys:
        ...
        if response_length > max_length:
            longest_response = response_content
            max_length = response_length

    result.append({
        "instruction": instruction,
        "response": longest_response
    })
...
print(f"The top 1000 instructions and their longest responses have been saved to {output_file}")